...
class CollectiveDatatablesviewsLayer(PloneSandboxLayer):

    defaultBases = (PLONE_FIXTURE, )
    """Layer to setup the datatablesviews site"""
    class Session(dict):
        def set(self, key, value):
            self[key] = value

    def setUpZope(self, app, configurationContext):
        """Set up the additional products required for the collective) site datatablesviews.
        ...
        z2.installProduct(app, 'collective.datatablesviews')
    ...
    def setUpPloneSite(self, portal):
        ...
        self.applyProfile(portal, 'collective.datatablesviews:default')

COLLECTIVE_DATATABLESVIEWS_FIXTURE = CollectiveDatatablesviewsLayer(name='CollectiveDatatablesviews:Fixture')

class LayerMixin(Layer):
    defaultBases = (COLLECTIVE_DATATABLESVIEWS_FIXTURE,)
...
COLLECTIVE_DATATABLESVIEWS_SIMPLE              = SimpleLayer(name='CollectiveDatatablesviews:Simple')
COLLECTIVE_DATATABLESVIEWS_INTEGRATION_TESTING = IntegrationTesting(name = "CollectiveDatatablesviews:Integration")
COLLECTIVE_DATATABLESVIEWS_FUNCTIONAL_TESTING  = FunctionalTesting( name = "CollectiveDatatablesviews:Functional")
COLLECTIVE_DATATABLESVIEWS_SELENIUM_TESTING    = FunctionalTesting(bases = (SELENIUM_TESTING,
                                                          COLLECTIVE_DATATABLESVIEWS_FUNCTIONAL_TESTING,),
                                                 name = "CollectiveDatatablesviews:Selenium")
...
class Browser(z2.Browser): # pragma: no cover
    ...
    def new(cls, url=None, user=None, passwd=None, headers=None, login=False):
        ...
        app = COLLECTIVE_DATATABLESVIEWS_FUNCTIONAL_TESTING['app']
        portal = COLLECTIVE_DATATABLESVIEWS_FUNCTIONAL_TESTING['portal']
        if not url:
